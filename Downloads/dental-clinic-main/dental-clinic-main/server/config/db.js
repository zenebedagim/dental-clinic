...
if (dbUrl.includes("neon.tech")) {
  ...
  if (process.env.DATABASE_URL_POOLER) {
    ...
    if (!connectionString.includes("connection_limit=")) {
      connectionString = addQueryParam(
        connectionString,
        "connection_limit",
        "5"
      );
...
if (process.env.NODE_ENV === "development") {
  prisma.$on("error", (e) => {
    ...
    if (e.message && e.message.includes("Closed")) {
      console.warn(
        "💡 Connection closed - Prisma will automatically reconnect on next query"
      );
    }
  });
}
...
if (process.env.VERCEL !== "1") {
  ...
  prisma
    ...
    .catch((error) => {
      ...
      if (dbUrl.includes("neon.tech")) {
        ...
        console.error(
          "1. Ensure your DATABASE_URL uses the connection pooling endpoint (ends with -pooler)"
        );
        console.error("2. Or set DATABASE_URL_POOLER with your pooler URL");
        console.error(
          "3. Verify SSL is enabled: connection string should include ?sslmode=require"
        );
        ...
        console.error(
          "6. Try using DATABASE_URL_DIRECT for migrations if pooler fails"
        );
      }
      ...
      console.error(
        '5. Test connection with: psql "$DATABASE_URL" or using a database client'
      );
  ...
  // Backup handler for normal exit
