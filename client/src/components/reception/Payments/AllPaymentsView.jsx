...
const AllPaymentsView = () => {
  ...
  const fetchPayments = useCallback(async () => {
    ...
    try {
      ...
      // Map payment data to display format and filter by branch (only visible payments)
      const formattedPayments = paymentsData
        .filter((payment) => {
          // Ensure payment belongs to the selected branch and is NOT hidden
          const paymentBranchId =
            payment.appointment?.branchId || payment.branchId;
          return paymentBranchId === selectedBranch.id && !payment.isHidden;
        })
        .map((payment) => ({
          id: payment.id,
  ...
  // Filter payments based on filters, branch isolation, and hidden payment toggle
  const filteredPayments = useMemo(() => {
    ...
    // Only show visible payments (not hidden) in AllPaymentsView
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3">
        <div>
          ...
          <p className="mt-2 text-sm text-gray-600 sm:text-base">
            Complete payment history for {selectedBranch.name}
          </p>
        </div>
