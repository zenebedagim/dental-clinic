...
const PrivatePaymentsView = () => {
  ...
  const fetchPayments = useCallback(async () => {
    ...
    try {
      ...
      // Map payment data to display format and filter by branch and isHidden
      const formattedPayments = paymentsData
        .filter((payment) => {
          // Ensure payment belongs to the selected branch and is hidden
          const paymentBranchId =
            payment.appointment?.branchId || payment.branchId;
          return (
            paymentBranchId === selectedBranch.id && payment.isHidden === true
          );
        })
        .map((payment) => ({
          id: payment.id,
